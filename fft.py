...
class GY521:
    ...
    def get_3axis_vector(self):
        try:
            # Read all 6 bytes (X, Y, and Z registers)
            ...
            # Combine all 3 axes into one magnitude vector
            magnitude_vector = math.sqrt(x**2 + y**2 + z**2)
            return magnitude_vector
        except Exception:
            return 1.0 # Baseline gravity approx if read fails
...
class FFTProcessor:
    ...
    def process_data(self, signal_data, actual_fs):
        ...
        # Look at frequencies starting from Bin 1 (0.39 Hz)
...
try:
    while True:
        ...
        for i in range(SAMPLES):
            ...
            # Gathers vector data from X, Y, and Z combined
        ...
        if total_duration_seconds > 0:
            ...
            if mag > 0:
                print("Tremor Freq: {:.2f} Hz | Intensity: {:.2f}".format(freq, mag))
            else:
                print("Tremor Freq: 0.00 Hz | Hand Steady")
